...
func (f *fakeSessionRepo) DeleteSessionSpeaker(ctx context.Context, sessionID, speakerID string) error {
	var kept []struct{ sessionID, speakerID string }
	for _, ss := range f.sessionSpeakers {
		if ss.sessionID != sessionID || ss.speakerID != speakerID {
			kept = append(kept, ss)
		}
	}
...
func (f *fakeSessionRepo) GetSpeakerByID(ctx context.Context, speakerID string) (*domain.Speaker, error) {
	for _, sp := range f.speakers {
		if sp.ID == speakerID {
			return sp, nil
		}
...
func (f *fakeSessionRepo) ListSpeakersByEventID(ctx context.Context, eventID string) ([]*domain.Speaker, error) {
...
func (f *fakeSessionRepo) ListSpeakersBySessionID(ctx context.Context, sessionID string) ([]*domain.Speaker, error) {
	speakerByID := make(map[string]*domain.Speaker)
	for _, sp := range f.speakers {
	...
	for _, ss := range f.sessionSpeakers {
		...
		if sp := speakerByID[ss.speakerID]; sp != nil {
			out = append(out, sp)
		}
	}
	if out == nil {
...
func (f *fakeSessionRepo) ListSessionIDsBySpeakerID(ctx context.Context, speakerID string) ([]string, error) {
	var out []string
	seen := make(map[string]bool)
	for _, ss := range f.sessionSpeakers {
		if ss.speakerID == speakerID && !seen[ss.sessionID] {
			seen[ss.sessionID] = true
			out = append(out, ss.sessionID)
		}
	}
	return out, nil
}

func (f *fakeSessionRepo) ListSessionsByIDs(ctx context.Context, sessionIDs []string) ([]*domain.Session, error) {
	if len(sessionIDs) == 0 {
		return []*domain.Session{}, nil
	}
	...
	speakerMap, _ := f.ListSpeakerIDsBySessionIDs(ctx, sessionIDs)
...
func (f *fakeSessionRepo) DeleteSpeaker(ctx context.Context, speakerID string) error {
	for i, sp := range f.speakers {
		if sp.ID == speakerID {
			f.speakers = append(f.speakers[:i], f.speakers[i+1:]...)
			var kept []struct{ sessionID, speakerID string }
			for _, ss := range f.sessionSpeakers {
				if ss.speakerID != speakerID {
					kept = append(kept, ss)
				}
			}
			f.sessionSpeakers = kept
...
func (f *fakeTagRepo) ListTagsByEventID(ctx context.Context, eventID string) ([]*domain.Tag, error) {
	tagIDs, ok := f.eventTags[eventID]
	if !ok {
		return nil, nil
	}
	var tags []*domain.Tag
...
func newTestEventService(eventRepo domain.EventRepository, sessionRepo domain.SessionRepository, fetcher domain.SessionFetcher, timeout time.Duration) *eventService {
	return NewEventService(
		eventRepo,
		sessionRepo,
		newFakeTagRepo(),
		newFakeEventTeamMemberRepo(),
		newFakeUserRepoForSchedule(),
		newFakeEventInvitationRepo(),
		newFakeEmailService(),
		fetcher,
		timeout,
	).(*eventService)
...
func TestEventService_CreateEventRoom(t *testing.T) {
	ctx := context.Background()
	timeout := 5 * time.Second
	...
	tests := []struct {
		...
		wantNotFound  bool
		wantForbidden bool
		assert        func(t *testing.T, room *domain.Room, sessionRepo *fakeSessionRepo)
	}{
		{
			name: "success creates room with expected fields",
			setup: func() (domain.EventRepository, domain.SessionRepository, domain.SessionFetcher) {
				er := newFakeEventRepo()
				_ = er.Create(ctx, &domain.Event{
					ID:        "ev-1",
					Name:      "Conf",
					OwnerID:   "user-1",
					CreatedAt: now,
			...
			notBookable:   false,
			assert: func(t *testing.T, room *domain.Room, sr *fakeSessionRepo) {
				require.NotNil(t, room)
				assert.Equal(t, "ev-1", room.EventID)
				assert.Equal(t, "Room A", room.Name)
				assert.Equal(t, 50, room.Capacity)
				assert.Equal(t, "Big room", room.Description)
				assert.Equal(t, "Floor 2", room.HowToGetThere)
				assert.Equal(t, false, room.NotBookable)
				assert.Equal(t, "admin_app", room.Source)
				assert.Equal(t, 0, room.SourceSessionID)
				// Ensure it was persisted via fakeSessionRepo
				require.Len(t, sr.rooms, 1)
				require.Equal(t, room.ID, sr.rooms[0].ID)
			},
		},
		{
			name: "event not found",
		...
		},
		{
			...
			setup: func() (domain.EventRepository, domain.SessionRepository, domain.SessionFetcher) {
				...
				sr := newFakeSessionRepo()
				sr.createRoomErr = errors.New("db failure")
				return er, sr, &fakeSessionizeFetcher{}
			},
			eventID:      "ev-1",
			ownerID:      "user-1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			eventRepo, sessionRepo, fetcher := tt.setup()
			sr, _ := sessionRepo.(*fakeSessionRepo)
			svc := NewEventService(eventRepo, sessionRepo, newFakeTagRepo(), newFakeEventTeamMemberRepo(), newFakeUserRepoForSchedule(), newFakeEventInvitationRepo(), newFakeEmailService(), fetcher, timeout)
			room, err := svc.CreateEventRoom(ctx, tt.eventID, tt.ownerID, tt.nameArg, tt.capacity, tt.description, tt.howToGetThere, tt.notBookable)
			if tt.wantErr {
				require.Error(t, err)
				if tt.wantNotFound {
					require.True(t, errors.Is(err, domain.ErrNotFound))
...
func TestEventService_CreateEventSession(t *testing.T) {
	ctx := context.Background()
	timeout := 5 * time.Second
	...
	tests := []struct {
		name          string
		setup         func() (domain.EventRepository, *fakeSessionRepo, domain.SessionFetcher, *fakeTagRepo)
		args          args
		wantErr       bool
		wantNotFound  bool
		wantForbidden bool
		wantInvalid   bool
		assert        func(t *testing.T, sr *fakeSessionRepo, tr *fakeTagRepo, sess *domain.Session)
	}{
		{
			name: "success with tags and speakers",
			setup: func() (domain.EventRepository, *fakeSessionRepo, domain.SessionFetcher, *fakeTagRepo) {
				er := newFakeEventRepo()
				_ = er.Create(ctx, &domain.Event{Name: "Conf", OwnerID: "user-1", CreatedAt: time.Now(), UpdatedAt: time.Now()})
				sr := newFakeSessionRepo()
			...
			assert: func(t *testing.T, sr *fakeSessionRepo, tr *fakeTagRepo, sess *domain.Session) {
				...
				assert.Equal(t, sess.ID, sr.sessions[0].ID)
				assert.Equal(t, "room-1", sess.RoomID)
				assert.Equal(t, "Talk", sess.Title)
				assert.Equal(t, "Desc", sess.Description)
				assert.True(t, sess.StartTime.Equal(start))
				assert.True(t, sess.EndTime.Equal(end))
				require.Len(t, tr.sessionTags, 1)
				tagIDs := tr.sessionTags[sess.ID]
				require.Len(t, tagIDs, 2)
				require.Len(t, sr.sessionSpeakers, 2)
				assert.Equal(t, sess.ID, sr.sessionSpeakers[0].sessionID)
				assert.Equal(t, "sp-1", sr.sessionSpeakers[0].speakerID)
				assert.Equal(t, sess.ID, sr.sessionSpeakers[1].sessionID)
				assert.Equal(t, "sp-2", sr.sessionSpeakers[1].speakerID)
			},
		},
		{
			name: "event not found",
			setup: func() (domain.EventRepository, *fakeSessionRepo, domain.SessionFetcher, *fakeTagRepo) {
				return newFakeEventRepo(), newFakeSessionRepo(), &fakeSessionizeFetcher{}, newFakeTagRepo()
			},
			args: args{
				eventID:   "ev-missing",
				ownerID:   "user-1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			eventRepo, sr, fetcher, tr := tt.setup()
			...
			got, err := svc.CreateEventSession(
				ctx,
				tt.args.eventID,
				tt.args.ownerID,
				tt.args.roomID,
			...
			require.NoError(t, err)
			if tt.assert != nil {
				tt.assert(t, sr, tr, got)
			}
		})
	}
}

func TestEventService_ListEventSpeakers(t *testing.T) {
	ctx := context.Background()
	timeout := 5 * time.Second
	...
	tests := []struct {
		...
		wantNotFound  bool
		wantLen       int
		assert        func(t *testing.T, speakers []*domain.Speaker)
	}{
		{
			name: "success returns speakers",
			setup: func() (domain.EventRepository, domain.SessionRepository, domain.SessionFetcher) {
				er := newFakeEventRepo()
				_ = er.Create(ctx, &domain.Event{Name: "Conf", OwnerID: "user-1", CreatedAt: time.Now(), UpdatedAt: time.Now()})
				sr := newFakeSessionRepo()
			...
			assert: func(t *testing.T, speakers []*domain.Speaker) {
				...
				assert.Equal(t, "ev-1", speakers[0].EventID)
			},
		},
		{
			name: "event not found",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.assert != nil {
				tt.assert(t, speakers)
			}
		})
	}
}

func TestEventService_GetEventSpeaker(t *testing.T) {
	ctx := context.Background()
	timeout := 5 * time.Second
	...
	tests := []struct {
		...
		{
			name: "success returns speaker and sessions",
			setup: func() (domain.EventRepository, domain.SessionRepository, domain.SessionFetcher) {
				er := newFakeEventRepo()
				_ = er.Create(ctx, &domain.Event{Name: "Conf", OwnerID: "user-1", CreatedAt: time.Now(), UpdatedAt: time.Now()})
				sr := newFakeSessionRepo()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			if tt.assert != nil {
				tt.assert(t, speaker, sessions)
			}
		})
	}
}

func TestEventService_DeleteEventSpeaker(t *testing.T) {
	ctx := context.Background()
	timeout := 5 * time.Second
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.assertDeleted {
				sr := sessionRepo.(*fakeSessionRepo)
				_, err := sr.GetSpeakerByID(ctx, tt.speakerID)
				require.True(t, errors.Is(err, domain.ErrNotFound), "speaker should be deleted")
			}
		})
	}
}

func TestEventService_CreateEventSpeaker(t *testing.T) {
	ctx := context.Background()
	timeout := 5 * time.Second
	...
	tests := []struct {
		...
		{
			name: "success creates speaker with generated sessionize_speaker_id",
			setup: func() (domain.EventRepository, domain.SessionRepository, domain.SessionFetcher) {
				er := newFakeEventRepo()
				_ = er.Create(ctx, &domain.Event{Name: "Conf", OwnerID: "user-1", CreatedAt: time.Now(), UpdatedAt: time.Now()})
				sr := newFakeSessionRepo()
...
func TestEventService_ListEventTags(t *testing.T) {
	ctx := context.Background()
	timeout := 5 * time.Second

	tests := []struct {
		name          string
		setup         func() (*fakeEventRepo, *fakeTagRepo)
		eventID       string
		callerID      string
		wantErr       bool
		wantForbidden bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			svc := NewEventService(
				...
				newFakeEventInvitationRepo(),
				newFakeEmailService(),
				&fakeSessionizeFetcher{},
				timeout,
			)
			tags, err := svc.ListEventTags(ctx, tt.eventID, tt.callerID)
			if tt.wantErr {
				require.Error(t, err)
				if tt.wantNotFound {
					require.True(t, errors.Is(err, domain.ErrNotFound))
...
func TestEventService_AddSessionSpeaker(t *testing.T) {
	ctx := context.Background()
	timeout := 5 * time.Second
	...
	tests := []struct {
		...
		wantForbidden bool
		wantNotFound  bool
		assert        func(t *testing.T, sr *fakeSessionRepo)
	}{
		{
			name: "success adds speaker to session",
			setup: func() (domain.EventRepository, *fakeSessionRepo, *fakeTagRepo) {
				er := newFakeEventRepo()
				_ = er.Create(ctx, &domain.Event{ID: "ev-1", Name: "Conf", OwnerID: "user-1", CreatedAt: time.Now(), UpdatedAt: time.Now()})
				sr := newFakeSessionRepo()
				sr.rooms = []*domain.Room{{ID: "room-1", EventID: "ev-1", Name: "Room A"}}
				sr.sessions = []*domain.Session{{ID: "sess-1", RoomID: "room-1", Title: "Talk"}}
				sr.speakers = []*domain.Speaker{{ID: "spk-1", EventID: "ev-1", FirstName: "Jane", LastName: "Doe"}}
				return er, sr, newFakeTagRepo()
			},
			eventID:   "ev-1",
			sessionID: "sess-1",
			ownerID:   "user-1",
			...
			assert: func(t *testing.T, sr *fakeSessionRepo) {
				var found bool
				for _, ss := range sr.sessionSpeakers {
					if ss.sessionID == "sess-1" && ss.speakerID == "spk-1" {
						found = true
						break
					}
				}
				require.True(t, found, "session_speakers should contain sess-1, spk-1")
			},
		},
		{
			name: "event not found",
		...
		},
		{
			...
			setup: func() (domain.EventRepository, *fakeSessionRepo, *fakeTagRepo) {
				er := newFakeEventRepo()
				_ = er.Create(ctx, &domain.Event{ID: "ev-1", Name: "Conf", OwnerID: "owner-1", CreatedAt: time.Now(), UpdatedAt: time.Now()})
				sr := newFakeSessionRepo()
				sr.rooms = []*domain.Room{{ID: "room-1", EventID: "ev-1", Name: "Room A"}}
				sr.sessions = []*domain.Session{{ID: "sess-1", RoomID: "room-1", Title: "Talk"}}
				sr.speakers = []*domain.Speaker{{ID: "spk-1", EventID: "ev-1"}}
				return er, sr, newFakeTagRepo()
			},
			eventID:       "ev-1",
			sessionID:     "sess-1",
		...
		},
		{
			...
			setup: func() (domain.EventRepository, *fakeSessionRepo, *fakeTagRepo) {
				...
				sr.rooms = []*domain.Room{{ID: "room-1", EventID: "ev-1", Name: "Room A"}}
				sr.sessions = []*domain.Session{{ID: "sess-1", RoomID: "room-1", Title: "Talk"}}
				sr.speakers = []*domain.Speaker{{ID: "spk-1", EventID: "ev-other"}}
				return er, sr, newFakeTagRepo()
			},
			eventID:      "ev-1",
			sessionID:    "sess-1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			er, sr, tr := tt.setup()
			svc := NewEventService(er, sr, tr, newFakeEventTeamMemberRepo(), newFakeUserRepoForSchedule(), newFakeEventInvitationRepo(), newFakeEmailService(), &fakeSessionizeFetcher{}, timeout)
			err := svc.AddSessionSpeaker(ctx, tt.eventID, tt.sessionID, tt.ownerID, tt.speakerID)
			if tt.wantErr {
				require.Error(t, err)
				if tt.wantNotFound {
					require.True(t, errors.Is(err, domain.ErrNotFound))
...
func TestEventService_RemoveSessionSpeaker(t *testing.T) {
	ctx := context.Background()
	timeout := 5 * time.Second
	...
	tests := []struct {
		...
		{
			name: "success removes speaker from session",
			setup: func() (domain.EventRepository, *fakeSessionRepo, *fakeTagRepo) {
				er := newFakeEventRepo()
				_ = er.Create(ctx, &domain.Event{ID: "ev-1", Name: "Conf", OwnerID: "user-1", CreatedAt: time.Now(), UpdatedAt: time.Now()})
				sr := newFakeSessionRepo()
				sr.rooms = []*domain.Room{{ID: "room-1", EventID: "ev-1", Name: "Room A"}}
				sr.sessions = []*domain.Session{{ID: "sess-1", RoomID: "room-1", Title: "Talk"}}
				sr.sessionSpeakers = []struct{ sessionID, speakerID string }{{"sess-1", "spk-1"}, {"sess-1", "spk-2"}}
				return er, sr, newFakeTagRepo()
			},
			eventID:   "ev-1",
			sessionID:  "sess-1",
			...
			assert: func(t *testing.T, sr *fakeSessionRepo) {
				for _, ss := range sr.sessionSpeakers {
					require.False(t, ss.sessionID == "sess-1" && ss.speakerID == "spk-1", "spk-1 should be removed from sess-1")
				}
				require.Len(t, sr.sessionSpeakers, 1)
				require.Equal(t, "sess-1", sr.sessionSpeakers[0].sessionID)
				require.Equal(t, "spk-2", sr.sessionSpeakers[0].speakerID)
			},
		},
		{
			name: "event not found",
		...
		},
		{
			...
			setup: func() (domain.EventRepository, *fakeSessionRepo, *fakeTagRepo) {
				...
				sr.rooms = []*domain.Room{{ID: "room-1", EventID: "ev-1", Name: "Room A"}}
				sr.sessions = []*domain.Session{{ID: "sess-1", RoomID: "room-1", Title: "Talk"}}
				sr.sessionSpeakers = []struct{ sessionID, speakerID string }{{"sess-1", "spk-1"}}
				return er, sr, newFakeTagRepo()
			},
			eventID:       "ev-1",
			sessionID:     "sess-1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			er, sr, tr := tt.setup()
			svc := NewEventService(er, sr, tr, newFakeEventTeamMemberRepo(), newFakeUserRepoForSchedule(), newFakeEventInvitationRepo(), newFakeEmailService(), &fakeSessionizeFetcher{}, timeout)
			err := svc.RemoveSessionSpeaker(ctx, tt.eventID, tt.sessionID, tt.ownerID, tt.speakerID)
			if tt.wantErr {
				require.Error(t, err)
				if tt.wantNotFound {
					require.True(t, errors.Is(err, domain.ErrNotFound))
...
func TestEventService_ListSessionSpeakers(t *testing.T) {
	ctx := context.Background()
	timeout := 5 * time.Second
	...
	tests := []struct {
		...
		{
			name: "success lists speakers linked to session",
			setup: func() (domain.EventRepository, *fakeSessionRepo, *fakeTagRepo) {
				er := newFakeEventRepo()
				_ = er.Create(ctx, &domain.Event{ID: "ev-1", Name: "Conf", OwnerID: "user-1", CreatedAt: time.Now(), UpdatedAt: time.Now()})
				sr := newFakeSessionRepo()
				...
				sr.sessions = []*domain.Session{{ID: "sess-1", RoomID: "room-1", Title: "Talk"}}
				sr.speakers = []*domain.Speaker{
					{ID: "spk-1", EventID: "ev-1", FirstName: "A"},
					{ID: "spk-2", EventID: "ev-1", FirstName: "B"},
				}
				sr.sessionSpeakers = []struct{ sessionID, speakerID string }{{"sess-1", "spk-1"}, {"sess-1", "spk-2"}}
				return er, sr, newFakeTagRepo()
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			er, sr, tr := tt.setup()
			svc := NewEventService(er, sr, tr, newFakeEventTeamMemberRepo(), newFakeUserRepoForSchedule(), newFakeEventInvitationRepo(), newFakeEmailService(), &fakeSessionizeFetcher{}, timeout)
			speakers, err := svc.ListSessionSpeakers(ctx, tt.eventID, tt.sessionID, tt.callerID)
			if tt.wantErr {
				require.Error(t, err)
				if tt.wantNotFound {
					require.True(t, errors.Is(err, domain.ErrNotFound))
